...
//@{
void 
MSVehicle::getNextAcceleration() const {
    SUMOTime currentTime = MSNet::getInstance()->getCurrentTimeStep();
    if (currentTime == lastGetAccelCall) {
        return;
    } else {
        if(nextAccelerations.empty()){
            calcAccelerateList();
        }
        lastGetAccelCall = currentTime;
        variableAccel = nextAccelerations.front();
        nextAccelerations.pop();
    }
}

void
MSVehicle::calcAccelerateList() const {
    double speed = getChosenBikeFactor() * avgBikeSpeed;
    if (speed < slowSpeed) {
        // Slow speed
        double loc = 0.150704827895694; 
        double scale = 0.10449190516706552;
        double a = -1.6754661187834592;
        double b = 1.218193030798051;
        for (int i = 0; i < TS; i++) {
            nextAccelerations.push(generateJohnsonSURandom(a, b, loc, scale));
        }
    } else if (speed > mediumSpeed) {
        // Fast Speed
        double loc = 0.19618587050964004;
        double scale = 0.12314145255505828;
        double a = -2.188462951458386;
        double b = 1.313856405799852;
        for (int i = 0; i < TS; i++) {
            nextAccelerations.push(generateJohnsonSURandom(a, b, loc, scale));
        }
    } else {
        // Medium Speed
        double loc = 0.1785814771087633;
        double scale = 0.11674245425817348;
        double a = -2.0187364732759256;
        double b = 1.270111030686134;
        for (int i = 0; i < TS; i++) {
            nextAccelerations.push(generateJohnsonSURandom(a, b, loc, scale));
        }
    }
...
double 
MSVehicle::generateJohnsonSURandom(double a, double b, double loc, double scale) const {
    double z = RandHelper::rand((double)1.0, getRNG());
    double x = loc + scale * sinh((z - a) / b);

    return x;
}

void 
MSVehicle::getNextDeceleration() const {
    SUMOTime currentTime = MSNet::getInstance()->getCurrentTimeStep();
    if(currentTime == lastGetDecelCall and currentDecel < 3){
        return;
    } else {
        if (stoppingPattern.empty()){
            calcStoppingList();
        }
        lastGetDecelCall = currentTime;
        currentDecel = stoppingPattern.front();
        stoppingPattern.erase(stoppingPattern.begin());
    }
}

void 
MSVehicle::calcStoppingList() const {
    double speed = getChosenBikeFactor() * avgBikeSpeed;
    if (speed < slowSpeed){
        //Slow speed
        double a = 2.053349401197436;
        double b = 1.2372282837024464;
        double loc = -0.12262805297027336;
        double scale = 0.06809583496952806;
        for (int i = 0; i < TS; i++){
            stoppingPattern.push_back(-generateJohnsonSURandom(a, b, loc, scale));
        }
    } else if (speed > mediumSpeed){
        //Fast Speed
        double a = 2.5381195379304833;
        double b = 1.3343276471994583;
        double loc = -0.15226543618916527;
        double scale = 0.08784696594180727;
        for (int i = 0; i < TS; i++){
            stoppingPattern.push_back(-generateJohnsonSURandom(a, b, loc, scale));
        }
    } else {
        //Medium Speed
        double a = 2.428287411173327;
        double b = 1.2792107332855989;
        double loc = -0.1382883038601964;
        double scale = 0.07614907226541474;
        for (int i = 0; i < TS; i++){
            stoppingPattern.push_back(-generateJohnsonSURandom(a, b, loc, scale));
        }
    }
