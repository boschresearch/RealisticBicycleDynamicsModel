...
private:
    /// @name Variables for bikes
    //@{
    /// @brief Bike's next Accelerations. One acceleration per timestamp 
    mutable std::queue<double> nextAccelerations;

    /// @brief Timestamp when getAcceleration was called last time. To check if new acceleration value is needed
    mutable SUMOTime lastGetAccelCall;

    /// @brief Bike's average speed in m/s which is needed to calculate maximum possible speed
    double avgBikeSpeed = 12.0;

    /// @brief The acceleration value of the bike which would be used for next action
    mutable double variableAccel = 0.0;
    ...
    /// @brief Bike's next decelerations in future. One deceleration per timestamp
    mutable std::vector<double> stoppingPattern;

    /// @brief Timestamp when getDeceleration was called last time. To check if new deceleration value is needed
    mutable SUMOTime lastGetDecelCall;

    /// @brief The deceleration value of the bike which would be used for next action
    mutable double currentDecel = 0.0;
    //@}

public:
    /// @name Methods for bikes
    //@{
    /**
     * @brief Set the next acceleration queue
    */
    inline void setNextAccelerationQueue(std::queue<double> nextAccels) const {
        nextAccelerations = nextAccels;
    }

    /**
     * @brief Get the acceleration queue
    */
    inline std::queue<double> getNextAccelerationQueue() const {
        return nextAccelerations;
    }

    /**
     * @brief Get the next acceleration of the vehicle's acceleration queue. If called more than once in a timestamp
     * it returns the same value
     * @return next usable acceleration
    */
    inline double getVariableAccel() const {
        getNextAcceleration();
        return variableAccel;
    }
    
    /**
     * @brief Gets the next acceleration of the acceleration queue and sets variableAccel to it
    */
    void getNextAcceleration() const;

    /**
     * @brief Calculates the next values for the acceleration queue based on the Johnson's SU distribution 
     * and vehicle's maximum speed
    */
    void calcAccelerateList() const;

    /**
     * @brief Generates a random value following the Johnson's SU distribution with the given parameters 
     * @return random value
    */
    double generateJohnsonSURandom(double a, double b, double loc, double scale) const;

    /**
     * @brief return the current stoppingPattern for the future
    */
    inline std::vector<double> getStoppingPattern() const {
        return stoppingPattern;
    }

    /**
     * @brief Gets the next deceleration of the deceleration vector and sets currentDecel to it
    */
    void getNextDeceleration() const;

    /**
     * @brief Calculates the next values for the deceleration vector based on the Johnson's SU distribution 
     * and vehicle's maximum speed
    */
    void calcStoppingList() const;

    /**
     * @brief return the next decel of the stopping pattern vector. If called more than once in a timestamp
     * it returns the same value
     * @return next usable deceleration
    */
    inline double getCurrentDecel() const{
        getNextDeceleration();
        return currentDecel;
    }

    /**
     * @brief Calculates the maximum reachable speed of the vehicle. With help of the SUMO parameter bikeFactor
     * @return vehicle's maximum speed
    */
    inline double getMaxSpeed() const {
        return myType->getMaxSpeed() * getChosenBikeFactor();
    }
    //@}
};
