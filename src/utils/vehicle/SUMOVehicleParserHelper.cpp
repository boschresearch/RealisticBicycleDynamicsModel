...
SUMOVehicleParserHelper::parseCommonAttributes(const SUMOSAXAttributes& attrs, const bool hardFail, SUMOVehicleParameter* ret, std::string element) {
    ...
    if (attrs.hasAttribute(SUMO_ATTR_BIKEFACTOR)) {
        double bikeFactor = attrs.get<double>(SUMO_ATTR_BIKEFACTOR, ret->id.c_str(), ok);
        if (bikeFactor > 0) {
            ret->parametersSet |= VEHPARS_BIKEFACTOR_SET;
            ret->bikeFactor = bikeFactor;
        } else {
            handleError(hardFail, abortCreation, toString(SUMO_ATTR_BIKEFACTOR) + " must be positive");
        }
    }
    /*/ parse via
...
SUMOVehicleParserHelper::getAllowedCFModelAttrs() {
    ...
    if (allowedCFModelAttrs.size() == 0) {
        ...
        allowedCFModelAttrs[SUMO_TAG_CF_BICYCLE_MODEL] = kraussParams;
